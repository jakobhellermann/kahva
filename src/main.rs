...
fn load() -> Result<MyApp> {
    ...
    for node in iter {
        ...
        let edges = graphlog_edges
            .iter()
            .map(convert_graph_edge_into_ancestor)
            .collect();
            ...
            for node in &content.nodes {
                ...
                if let Some(term_row) = &node.row.term_line {
                    ...
                    for (i, _) in term_row.iter().enumerate() {
                        ...
                        /*for i in rect.center_top().y..rect.center_bottom().y {
                        }*/
                    }
                }
        ...
        ui.horizontal(|ui| {
            ...
            for (i, line) in node_line.iter().enumerate() {
                ...
                let is_head = match prev_link_line.and_then(|l| l.get(i)) {
                    ...
                    None => false,
                    Some(link_line) => {
                        !link_line.intersects(LinkLine::ANY_FORK | LinkLine::VERT_PARENT)
                    }
                };
...
fn get_node_template(settings: &UserSettings) -> Result<Cow<'static, str>, ConfigGetError> {
    ...
    Ok(symbol
        .map(Cow::Owned)
        .unwrap_or(Cow::Borrowed("builtin_log_node")))
}
